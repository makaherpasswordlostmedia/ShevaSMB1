...
#ifdef ANDROID
    // На Android SDLActivity может уже открыть аудио-устройство.
    // Mix_OpenAudio в таком случае вернёт ошибку — делаем её non-fatal.
    // Буфер 512 — стандарт для мобильных (1024 вызывает задержки).
    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 512) != 0) {
        SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO,
                    "Mix_OpenAudio failed (non-fatal on Android): %s", Mix_GetError());
        // Продолжаем без звука — не крашимся
        return 0;
    }
#else
...
void SoundManager::playMusic(MusicID music) {
    stopMusic();
    auto* mus = musics.at(music).get();
    if (mus) Mix_PlayMusic(mus, -1);
}
