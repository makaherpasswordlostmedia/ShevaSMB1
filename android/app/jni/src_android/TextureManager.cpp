...
#ifdef ANDROID
    // SDLActivity (Java) уже инициализировала SDL и создала окно до вызова main().
    // Повторный SDL_Init(SDL_INIT_VIDEO) и SDL_CreateWindow на Android — это краш.
    // Инициализируем только то, что ещё не запущено.
...
#ifndef ANDROID
    // На Android окно принадлежит SDLActivity — не уничтожаем его вручную
...
void TextureManager::DrawVerticalFlipped(std::shared_ptr<SDL_Texture> texture,
                                         SDL_Rect sourceRect, SDL_Rect destRect) {
    SDL_RenderCopyEx(renderer, texture.get(), &sourceRect, &destRect, 0, nullptr,
                     SDL_FLIP_VERTICAL);
}
...
#endif
    float scaleWidth  = (float)windowWidth  / (float)SCREEN_WIDTH;
    float scaleHeight = (float)windowHeight / (float)SCREEN_HEIGHT;
